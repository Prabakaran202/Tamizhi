...
// 🌟 ஹெடர் பிரீ-ஸ்கேன் (Pre-Scan): 
// கோப்பு முழுக்க தேடி பயனர் எழுதியுள்ள பங்க்ஷன் (fun) பெயர்களை மட்டும் முன்கூட்டியே ரிஜிஸ்டர் செய்யும் இடம்
void scan_headers(FILE *file) {
    ...
    while ((t = get_next_token(file)).type != T_EOF) {
        // 'fun' அல்லது தமிழ் கீவேர்ட் 'நிகழ்' வார்த்தையைப் பார்த்தால் உள்ளே நுழையும்
        if (strcmp(t.value, "fun") == 0 || t.type == T_FUNC || strcmp(t.value, "நிகழ்") == 0) {
            ...
            if (is_valid(name)) {
                ...
                // மெயின் பிளாக்கைத் தவிர்த்து மற்ற தனி பங்க்ஷன்களை மட்டும் கம்பைலருக்கு காட்டுகிறது
                ...
                while ((skip_t = get_next_token(file)).type != T_EOF) {
                    ...
                    // பிராக்கெட் ஓபன் ஆவதற்குள் அடுத்த 'fun' வந்தால் லூப் உடையாமல் இருக்க பாயிண்டரை ரீசெட் செய்கிறது
    ...
    rewind(file); // பிரீ-ஸ்கேன் முடிந்து மெயின் எக்ஸிகியூஷன் படிக்க ரீவைண்ட் செய்யப்படுகிறது
}

// 🌟 பிரதான பார்ஸர் இன்ஜின் (Main Parser Engine):
// குறியீட்டின் 'main' மற்றும் 'footer' பிளாக்குகள் எங்குள்ளது என்று கண்டறிந்து இயக்கும் இடம்
void parse(FILE *file) {
    ...
    // Phase 1: குளோபல் பங்க்ஷன்களை முன்கூட்டியே ஸ்கேன் செய்து ரிஜிஸ்டர் செய்தல்
    scan_headers(file);

    // Phase 2: மெயின் மற்றும் பூட்டர் பிளாக்குகள் எங்குள்ளது என்று பொசிஷனை மட்டும் கண்டறிந்து லாக் செய்தல்
    rewind(file);
    while ((t = get_next_token(file)).type != T_EOF) {
    ...
    // Phase 3: மெயின் (Main) பகுதிக்குள் இருப்பவற்றை முதலில் இயக்குதல்
    ...
    // Phase 4: பூட்டர் (Footer) பகுதிக்குத் தாவி வரிசையாக இயக்குதல்
    if (footer_pos != -1L) {
        ...
        while (footer_brace_count > 0 && (t = get_next_token(file)).type != T_EOF) {
            ...
            parse_statement(file, t); // பூட்டர் பிளாக்குக்குள் இருக்கும் ஃபங்ஷன் கால்கள் ஒவ்வொன்றாக இயங்கும்
        }
    }
...
// 🌟 ஸ்டேட்மென்ட் அனலைசர் (Statement Analyzer): 
// ஒவ்வொரு தனித்தனி வரிகளையும் (Num, Str, ID, Print, For, If-Else) பகுப்பாய்வு செய்யும் பிரதான இடம்
void parse_statement(FILE *file, Token t) {
    ...
    // 3️⃣ ஐடென்டிஃபையர் பிளாக் (Identifier Block): வேரியபிள் கணக்கீடு அல்லது ஃபங்ஷன் கால்களைக் கையாளுதல்
    else if (t.type == T_ID) {
        char var_name[50];
        ...
        // 🅰️ '=' அல்லது தமிழ் குறியீடு இருந்தால் கணித ஆபரேஷன்கள் (+, -, *, /)
        ...
        // 🅱️ '(' இருந்தால் அது பூட்டர் வழியா நடக்குற ஒரு பங்க்ஷன் கால் (Function Call Flow)
            ...
            // 🌟 லோக்கல் ஸ்கோப் மெமரி மேனேஜ்மென்ட் பிக்ஸ்: பங்க்ஷனுக்குள் இருக்கும் வேரியபிள்களை லோக்கலாக கையாள்கிறது
            if (func_body_pos != -1L) {
                clearerr(file);
                ...
                int previous_var_count = var_count; // 🌟 முந்தைய வேரியபிள் கவுண்ட்டை லாக் செய்கிறது
    ...
    // 4️⃣ லூப் பிளாக் (for / சு) -> லூப்களைக் கையாளுவதற்கு
    ...
    // 5️⃣ அச்சிடு பிளாக் (print ;) -> திரையில் அவுட்புட் காட்ட
    ...
    // 6️⃣ புதிய இஃப்-எல்ஸ் பிளாக் (if / எனில்) -> கண்டிஷனல் கண்ட்ரோல் ஃப்ளோ லாஜிக் 🌟
    else if (strcmp(t.value, "if") == 0 || strcmp(t.value, "எனில்") == 0) {
        Token open_bracket = get_next_token(file); // '(' ரீட் செய்கிறது
        Token v1 = get_next_token(file);           // முதல் வேரியபிள்
        Token op = get_next_token(file);           // கம்பாரோசன் ஆபரேட்டர் (<, >, ==, !=)
        Token v2 = get_next_token(file);           // இரண்டாம் மதிப்பு
        Token close_bracket = get_next_token(file); // ')' ரீட் செய்கிறது
        
        tamizhi_gen_if_start(v1.value, op.value, v2.value); // எல்எல்விஎம் 'then' மற்றும் 'else' பிளாக்குகளை உருவாக்குகிறது
        ...
        if (next.type == 22 || strcmp(next.value, "{") == 0) {
            ...
            while (if_brace_count > 0 && (if_body_t = get_next_token(file)).type != T_EOF) {
                ...
                if (if_body_t.type == 23 || strcmp(if_body_t.value, "}") == 0) {
                    if_brace_count--;
                    if (if_brace_count <= 0) break; // இஃப் பாடி முடிந்தது
                }
        ...
        // ஒருவேளை பயனர் 'else' அல்லது 'இல்லை எனில்' பிளாக் எழுதியிருந்தால் மட்டும் உள்ளே நுழையும்
        if (strcmp(else_t.value, "else") == 0 || strcmp(else_t.value, "இல்லை எனில்") == 0) {
            tamizhi_gen_else_start(); // எல்எல்விஎம் பில்டரை 'else' பிளாக்கிற்குள் நகர்த்துகிறது
            ...
            if (else_brace.type == 22 || strcmp(else_brace.value, "{") == 0) {
                ...
                while (else_brace_count > 0 && (else_body_t = get_next_token(file)).type != T_EOF) {
                    ...
                    if (else_body_t.type == 23 || strcmp(else_body_t.value, "}") == 0) {
                        else_brace_count--;
                        if (else_brace_count <= 0) break; // எல்ஸ் பாடி முடிந்தது
                    }
                    parse_statement(file, else_body_t);
                }
            }
        } else {
            fseek(file, else_pos, SEEK_SET); // 'else' பிளாக் இல்லையெனில் பாயிண்டரை பழைய இடத்திற்கே திருப்புகிறது
        }
        tamizhi_gen_if_end(); // எல்எல்விஎம் பில்டரை பாதுகாப்பாக 'merge' பிளாக்கிற்குள் கொண்டு வந்து சீக்வென்ஷியலா மாத்துகிறது!
    }
}
